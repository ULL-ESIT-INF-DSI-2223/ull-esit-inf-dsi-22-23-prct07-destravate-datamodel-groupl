...
describe('Clase User', () => {
  ...
  User.idLastUser(0);
  it("Setter del id del usuario resulta 1", () => {
    user1.setId()
  ...
  it("Setter de la ruta favorita añadiendo la 1, resulta [1]", () => {
    user1.setFavouriteRoute(1);
    expect(user1.favouriteRoutes).to.be.eql([1]);
  });

  it("Setter de reto favoraito se añade el 1 y el 4, resulta [1,4]", () => {
    user1.setChallenge(1);
    user1.setChallenge(4);
    expect(user1.challenges).to.be.eql([1,4]);
  });
